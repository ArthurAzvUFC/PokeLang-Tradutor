...
## [cite_start]1. Introdução [cite: 6, 8, 9]
...
## [cite\_start]3. Guia de Sintaxe e Comandos [cite: 7]
...
## [cite\_start]4. Implementação Técnica (O Tradutor) [cite: 10, 14]
...
2.  **`pokego.py` (A CLI):** É a interface de linha de comando. Utiliza a biblioteca `argparse` para gerenciar argumentos, chama o motor de tradução, executa o código Python gerado via `subprocess` e realiza a limpeza de arquivos temporários.

---

## [cite\_start]5. Códigos Exemplo [cite: 16, 17, 18, 19, 20, 21]
...
## [cite\_start]6. Como Executar [cite: 22]

Para rodar os programas, é necessário ter o Python 3 instalado.

1.  Coloque os arquivos `pokego.py` e `pokelang.py` na mesma pasta do seu código.
...
**Comando para Debug (Ver o código Python gerado):**

```bash
python pokego.py quiz.poke --debug
```

**Ver Ajuda:**

```bash
python pokego.py --help
```
